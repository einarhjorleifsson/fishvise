...
#' 
#' Original function from Hoski. This function is used in \code{\link{bioIndex.Original}}.
...
#' bioIndex.Original
#' 
#' Original long stuff in SMB BIOVISIT_R shell script. Runs in a loop for each length class
#' 
#' @export
#' @param lengths Data frame containing length information
#' @param stations Data frame containg station information
#' @param KYN kyn
#' @param lengdir Lengdir
#' @param yr Year
bioIndex.Original <- function(lengths,stations,KYN,lengdir,yr) {
  for( i in 1:length(lengdir)) {
    
    tmp1 <- lengths[lengths$lengd==lengdir[i],]
    ...
    if(KYN) {
      tmp1 <- lengths[lengths$lengd==lengdir[i] & lengths$kyn==1 & !is.na(lengths$kyn),]
      if(nrow(tmp1) > 0) {
        x <- apply.shrink(tmp1$fj.alls,tmp1$synis.id,sum) 
      ...
      tmp1 <- lengths[lengths$lengd==lengdir[i] & lengths$kyn==2 & !is.na(lengths$kyn),]
      if(nrow(tmp1) > 0) {
        x <- apply.shrink(tmp1$fj.alls,tmp1$synis.id,sum) 
    ...
    tmp.visit1$ar <- rep(yr,nrow(tmp.visit1))
    tmp.visit2$ar <- rep(yr,nrow(tmp.visit2))
  ...
  outdata <- list(base=tmp.visit1a,aggr=tmp.visit2a)
  return(outdata)
}
...
#' 
#' @export
#' @param Lengths data.frame containing lengths 
#' @param Numbers data.frame containing numbers 
#' @param col.names A character vector containing field name of n lengths, and n counted
#' @param id.name the name that joins Numbers to Lengths, default synis.id
qCounted <- function (Lengths,Numbers,col.names=c('fj.maelt','fj.talid'),id.name=c('synis.id')) {
  Numbers$rat <- 1 + Numbers[,col.names[1]]/Numbers[,col.names[2]]
  Numbers$rat[is.na(Numbers$rat)] <- 1
  i <- match("rat", names(Lengths))
  if (!is.na(i)) Lengths <- Lengths[, -i]
  l1 <- plyr::join(Lengths, Numbers[, c(id.name, "rat")],id.name)
  i <- is.na(l1$rat)
  if (any(i)) l1$rat[i] <- 1
  Lengths$n <- l1$fjoldi * l1$rat
  return(Lengths)
}
...
#' @export
#' @param stodvar xx
#' @param stratalist ralllist
inside.strata <-
function (stodvar, stratalist) 
{
  stodvar$newstrata <- rep(0, nrow(stodvar))
  for (i in stratalist) {
    j <- geoinside(stodvar, reg = STRATAS[[i]], robust = T, 
                   option = 0)
    if (length(j) > 0) 
