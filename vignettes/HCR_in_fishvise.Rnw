...
file <-  paste(path.package("fishvise"),'extdata/hcr_iCod_short.ctr',sep='/')
ctr <- hcr_read_ctr(file)
@
  \item Set the management harvest rate vectors to be tested and add it to the control object:
<<quick_start2>>=
HRATE <- c(0.20,0.30)
ctr$HRATE <- HRATE
@
